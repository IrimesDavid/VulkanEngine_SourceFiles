...
namespace lve {
	...
	std::unique_ptr<LveModel> LveModel::createModelFromFile(LveDevice& device, const std::string& filepath) {
		...
		auto model = std::make_unique<LveModel>(device, builder);

		// Load materials after model is created
		...
		for (auto& mat : materials) {
			if (!mat.diffuse_texname.empty()) {
				...
				model->fragmentBuffer.diffuseTexture = LveTexture::createFromFile(device, fullPath);
			}
		}
	...
	//NOTE: adapt the model from standard OpenGl: X=right, Y=up, -Z=forward to Vulkan: X=right, -Y=up, Z=forward
