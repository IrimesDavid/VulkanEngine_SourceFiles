...
namespace lve {
    ...
    void LveDevice::pickPhysicalDevice() {
        ...
        // First, try to find a suitable NVIDIA GPU
        for (const auto& device : devices) {
            VkPhysicalDeviceProperties properties;
            vkGetPhysicalDeviceProperties(device, &properties);
            if (isDeviceSuitable(device) && strstr(properties.deviceName, "NVIDIA") != nullptr) {
                physicalDevice = device;
                break;
            }
        }

        // If no NVIDIA GPU was found, pick the first suitable device
        ...
        // After picking the physical device, query supported sample counts
        VkPhysicalDeviceProperties physicalDeviceProperties;
        vkGetPhysicalDeviceProperties(physicalDevice, &physicalDeviceProperties);

        supportedSampleCounts = physicalDeviceProperties.limits.framebufferColorSampleCounts &
                                physicalDeviceProperties.limits.framebufferDepthSampleCounts;

        // Set default sample count (assuming 2 samples as default if possible)
